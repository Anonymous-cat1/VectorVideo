...
**Anonymous_cat1 is not responsible for any action taken by the Scratch website or the Scratch Team that could result from uploading copyrighted/illegal content. Nor any damage caused by the compiler or client if it breaks.**
## Index
- [Client](#client)
- [Compiler](#compiler)
- [Installation](#installation)
- [Basic Usage](#basic-usage)
- [Using the Estimator](#using-the-estimator-and-how-to-interpret-it)
...
## Compiler
A powerful Python toolkit for compiling videos into a format playable by Vector Video clients. This compiler turns videos into a series of compact JPEG atlases and audio chunks to enable vanilla Scratch3 VMs to play high-resolution video with little CPU cost.
...
# Use with esitmator (does not convert entire video)
```
...
```
### Estimator tips:
To use the estimator to its advantage:
...
3. Upload audio to the player sprite and save. **It is not required to upload them in any order.** 
> NOTE: For faster uploading, you may want to skip the frame blocks and upload all frames at once.
> 
> To do this,  search `.jpg` in your video's export directory, and upload all frames. While uploading, use an autoclicker to click `Save Now` to save in small chunks in order to avoid `Project cannot be saved!` errors.
4. Upload frames to the player sprite. Open a frame block folder, then upload its contents, then save before moving to the next frame block folder. **It is not required to upload them in any order.** 
...
> NOTE: Scratch's backend can sometimes create a "phantom project" which **you cannot delete** if your file is abnormally large (+1 GB). 

> COMPATIBILITY NOTE: The compiler does not check if the project is uploadable. There may be times when the compiler creates a project that cannot be uploaded to Scratch. See [Limitations](#Limitations) for more information.
...
- Due to using .JPG files instead of a custom renderer like most long-form video players on Scratch do, RAM usage is a major concern.
- **Again, RAM is your biggest enemy, so pay attention to it! You may crash your, or someone else's, system!**
